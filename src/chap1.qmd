...
The factor models introduced in @sec-factor-models can predict held-out entries in the response matrix when trained with appropriate masking schemes (see @sec-generalization in @sec-learning). But what if we want to evaluate a *brand-new* model or benchmark item that has never been tested? This is the **cold-start problem**: predicting correctness without any observed responses for the target model or item.
...
**Stage 1 — Factor Model Pretraining.** We first learn latent behavioral factors $(U, V, Z)$ from observed response data $Y_{ij}$ using the factor model from @sec-factor-models:
