...
| $\Phi(x)$ | Standard normal CDF; also a generic monotone link in @sec-acm | $(0,1)$ | @sec-foundations |
...
| $\boldsymbol{\beta}_j$ | Item-parameter vector $(\beta_{j1}, \beta_{j2}, \beta_{j3})$ = (difficulty, discrimination, guessing); $\beta_{j1} = \beta_j$ | $\mathbb{R}^n$ | @sec-foundations |
| $p_{ij}$, $\hat{p}_{ij}$ | Response probability $P(Y_{ij}=1)$ and its empirical estimate | $[0,1]$ | @sec-foundations |
| $f, g$ | Additive conjoint component functions (ability, difficulty factors) | $\mathbb{R}$ | @sec-foundations |
| $\beta_{j2}$ | Discrimination parameter of item $j$ (component of $\boldsymbol{\beta}_j$; conventionally $a_j$) | $\mathbb{R}^+$ | @sec-foundations |
| $\beta_{j3}$ | Guessing (pseudo-chance) parameter of item $j$ (component of $\boldsymbol{\beta}_j$; conventionally $c_j$) | $[0,1]$ | @sec-foundations |
...
| Symbol | Meaning | Domain | Introduced |
|--------|---------|--------|------------|
| $\mathcal{C}$ | Construct: the latent capability being measured | --- | @sec-what-to-hide |
| $D$ | Operationalizing distribution (formally $\pi_E$) | distribution on $F$ | @sec-what-to-hide |
| $D', D''$ | Public proxy distributions for $\mathcal{C}$ | distributions on $F$ | @sec-cv-protocol |
| $S$ | Realized evaluation set, $S \sim D$ | $S \subseteq F$ | @sec-what-to-hide |
| $\mu$ | Social-relevance distribution (construct fully operationalized; $\text{Uniform}(F)$ as a special case) | distribution on $F$ | @sec-what-to-hide |
...
| $u_E(\theta)$ | Evaluator's utility: $\mathbb{E}_{f \sim \mu}[f(\theta)]$ | $\mathbb{R}^+$ | @sec-evaluation-game |
| $k, k^*$ | Number of (optimal) sampled evaluation tasks per round | $\mathbb{N}$ | @sec-evaluation-game |
| $\rho, \sigma_c$ | Distribution correction rate and bandwidth | $(0,1] \times \mathbb{R}^+$ | @sec-evaluation-game |
| $\Delta_t$ | Residual misalignment: $\max_\theta u_E(\theta) - u_E(\theta_t)$ | $\mathbb{R}^+$ | @sec-evaluation-game |
| $\tilde{\pi}_E$ | Builder's belief about the evaluator's sampling | distribution on $F$ | @sec-evaluation-game |
